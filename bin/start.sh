...
SERVER_NAME=`sed '/server.name/!d;s/.*=//' conf/application.yml | tr -d '\r'`
SERVER_PORT=`sed '/server.port/!d;s/.*=//' conf/application.yml | tr -d '\r'`
LOGS_FILE=`sed '/server.log4j.file/!d;s/.*=//' conf/application.yml | tr -d '\r'`
...
CORE_JARS=`ls $CORE_DIR|grep .jar|awk '{print "'$CORE_DIR'/"$0}'|tr "\n" ":"`
...
nohup $JAVA_HOME/bin/java $JAVA_OPTS $JAVA_E_OPTS $JAVA_DEBUG_OPTS $JAVA_JX_OPTS -classpath \
    $CONF_DIR:$CORE_JARS:$LIB_JARS com.air.starter.Starter > $STDOUT_FILE 2>&1 &
